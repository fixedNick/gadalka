...
class Article {
    ...
    public function TotalWords() {
        $totalWords = 0;
        foreach ($this->Body as $k => $part) {
            $words = sizeof(explode(' ', $part->Content));
            $totalWords += $words;
        }
        return $totalWords;
    }
    ...
    public static function GetArticleList($limit = 10, $by = 'create_timestamp', $cat = null) {
        $m = new mysqli(getenv('dbhost'),getenv('dbuser'), getenv('dbpass'), getenv('dbname'));
        if($m->connect_error) {
            die($m->connect_error);
        }
    ...
    public static function GetPopularArticles($limit) {
        $articles = Article::GetArticleList($limit, 'views');
        return $articles;
    }
